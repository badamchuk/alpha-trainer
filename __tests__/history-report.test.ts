...
suite('звіт по реальній історії', () => {
  it("рахує покриття, калорії й об'єм", () => {
    ...
    console.log([
      ...
      `підходи за тиждень: ${volume.filter((v) => v.weeklySets > 0).map((v) => `${v.label} ${v.weeklySets}`).join(', ') || '—'}`,
      `підходи за тиждень (з бібліотекою): ${volumeR.filter((v) => v.weeklySets > 0).map((v) => `${v.label} ${v.weeklySets}`).join(', ') || '—'}`,
      `баланс м'язів (усього підходів): ${balance.map((b) => `${b.label} ${b.count}`).join(', ') || '—'}`,
      `баланс м'язів (з бібліотекою): ${balanceR.map((b) => `${b.label} ${b.count}`).join(', ') || '—'}`,
